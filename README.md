...
```bash
METRIC=UTM DEVICE=cuda:0 DEPTHS=1-65 SEARCH_DEPTHS=1000 TRAIN_PER_DEPTH=1 VAL_PER_DEPTH=1000 scripts/01_make_datasets.sh
DEVICE=cuda:0 STATE_DEPTHS=0-65 STATE_EPOCHS=1024 STATE_LR=0.00002 STATE_STEPS_PER_EPOCH=51 STATE_BATCH=20000 scripts/02_train_state.sh
DEVICE=cuda:0 NEIGH_DEPTHS=1-65 NEIGH_EPOCHS=4096 NEIGH_LR=0.00002 NEIGH_STEPS_PER_EPOCH=4 NEIGH_BATCH=8192 scripts/05_train_neighbour.sh
...
`UTM` и `FTM` используют разные validation/test/search датасеты, разные teacher labels для validation и разные `latest_*` чекпоинты. Во время обучения `State Model` и `Neighbour Model` генерируют новые random walks каждую эпоху; сохраненный `state_train.pt` задает только метрику и список глубин, поэтому `TRAIN_PER_DEPTH=1` достаточно. По умолчанию model-level датасеты используют `DEPTHS=1-65`, а solver-level `search_test.pt` генерируется с фиксированной глубиной `SEARCH_DEPTHS=1000`. `State Model` учится на `STATE_DEPTHS=0-65` сбалансированным набором глубин, как public CayleyPy value trainer; `Neighbour Model` учится на `NEIGH_DEPTHS=1-65`, как public CayleyPy Q trainer. Длины решений сравниваются только внутри одной метрики ходов.
...
`scripts/08_beam_sweep.sh` пишет JSON для каждого `B`, CSV `logs/beam_sweep_<metric>.csv` и PNG `logs/beam_sweep_<metric>.png` с графиками `length~B`, `length~time`, `time~B`, `length~FLOPs`. Именно JSON/CSV из sweep на `SEARCH_DEPTHS=1000` и `logs/superflip_*` стоит использовать как основные таблицы для диплома.
