...
```bash
METRIC=UTM DEVICE=cuda:0 DEPTHS=1-65 TRAIN_PER_DEPTH=1 VAL_PER_DEPTH=1000 scripts/01_make_datasets.sh
...
`UTM` и `FTM` используют разные validation/test/search датасеты, разные teacher labels для validation и разные `latest_*` чекпоинты. Во время обучения `State Model` и `Neighbour Model` генерируют новые random walks каждую эпоху; сохраненный `state_train.pt` задает только метрику и список глубин, поэтому `TRAIN_PER_DEPTH=1` достаточно. По умолчанию датасеты используют `DEPTHS=1-65`, `State Model` учится на `STATE_DEPTHS=0-65` сбалансированным набором глубин, как public CayleyPy value trainer; `Neighbour Model` учится на `NEIGH_DEPTHS=1-65`, как public CayleyPy Q trainer. Длины решений сравниваются только внутри одной метрики ходов.
